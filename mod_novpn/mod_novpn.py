...
def configure_openvpn_server(SERVER_PUBLIC_URL,PKI_ADDRESS,DNS_SERVER):
    ...
    try:
        ...
        if not os.path.exists('/etc/openvpn/pki/crl.pem'):
            try:
                COMMAND=f'/usr/local/bin/ovpn_genconfig -u {SERVER_PUBLIC_URL} -n {DNS_SERVER} -E "cipher none"'
...
def read_configuration():
    ...
    CHANNEL = config['REDIS']['REDIS_NOVPN_CHECK']
    LOG_FILE = config['LOGS']['LOG_NOVPN']
    SERVER_PUBLIC_URL = config['NOVPN']['SERVER_PUBLIC_URL']
    PKI_ADDRESS = config['NOVPN']['PKI_ADDRESS']
...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename=LOG_FILE, level=logging.DEBUG,format='%(asctime)s, MOD_NOVPN, %(message)s')
        ...
        for item in db_subscriber.listen():
            ...
            if item['type'] == 'message':
                ...
                if item['data'] == 'report_status':
                    redis_client.publish('services_status', 'MOD_NOVPN:online')
                    ...
                    CLIENT_IP=get_vpn_client_ip_address('novpn',redis_client)
                    if not CLIENT_IP==False:
                        # Parse the name obtained in the request
                        CLIENT_NAME=item['data'].split(':')[1]
                        redis_client.publish('services_status',f'MOD_NOVPN: assigning IP ({CLIENT_IP}) to client ({CLIENT_NAME})')
                        ...
                        if generate_openvpn_profile(CLIENT_NAME):
                            ...
                            if add_profile_ip_relationship(CLIENT_NAME,CLIENT_IP,redis_client):
                                ...
                                account_error_message="MOD_NOVPN: profile_creation_failed:cannot add profile_ip relationship to redis"
                        else:
                            account_error_message="MOD_NOVPN: profile_creation_failed:failed to create a new profile"
                    else:
                        account_error_message="MOD_NOVPN: profile_creation_failed:no available IP addresses found"
