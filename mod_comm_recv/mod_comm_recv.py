...
def send_request_to_redis(msg_id, msg_addr, msg_type, msg_request, logging, redis_client):
    ...
    """
    try:
        logging.debug("Sending %s request to Redis: (%s) {%s} on {%s}",
                      msg_request,
                      str(msg_id),
                      msg_addr,
                      msg_type)
        ...
        logging.info("Exception in send_request_to_redis: %s", err)
        return False
...
def get_telegram_requests(redis_client, TELEGRAM_BOT_TOKEN, TELEGRAM_START_MSG, TELEGRAM_WAIT_MSG):
    ...
    def telegram_cmd(update, context, msg_request):
        ...
        logging.info("New Telegram %s request received from: %s",
                     msg_request,
    ...
    try:
        ...
        openvpn_handler = CommandHandler('getopenvpn',
                                         partial(telegram_cmd, "openvpn"))
        dispatcher.add_handler(openvpn_handler)

        wireguard_handler = CommandHandler('getwireguard',
                                           partial(telegram_cmd, "wireguard"))
        dispatcher.add_handler(wireguard_handler)

        novpn_handler = CommandHandler('getnoencryptedvpn',
                                       partial(telegram_cmd, "novpn"))
        ...
        logging.debug("Exception in get_telegram_requests: %s", err)


def open_imap_connection():
    """
    Functions opens a connection to an email service,
    logs in to specified email account, and returns a connection.
    """
...
def select_inbox_messages():
    ...
    try:
        ...
        # Connect to Inbox
        # Readyonly option:
        #     False=marks msgs as read;
        #     True=keep messages as unread.
        ...
        if status == "OK":
            ...
            if len(id_msg_list) > 0:
                ...
                for msg_id in id_msg_list:
                    logging.info("Processing message %s", msg_id)
                    # Fetch the email headers and body (RFC822)
                    # for the given email UID
                    typ, data = mail.uid('fetch', msg_id, '(RFC822)')
                    msg = data[0]
                    yield msg_id, msg
    except Exception as err:
        logging.error("Exception in select_inbox_messages function %s", err)
        return False
    finally:
        mail.expunge()
        mail.close()
...
def search_for_vpn_keyword(msg):
    ...
    for pattern in patterns:
        try:
            ...
            if match:
                logging.info("Found the correct match for vpn keyword: %s", match.group(0))
                return match.group(0)
        except Exception as err:
            logging.error("Failed to find the correct email subject in parse_email_subject %s", err)
            return False
...
def get_email_body_data(message):
    ...
    try:
        ...
        logging.debug("Failed with exception %s. Email body is not in rich email.", err)
    ...
    try:
        ...
        for email_uid, inbox_message in messages:
            ...
            if msg_request:
                ...
                logging.info("Extracted email request: %s: %s (%s)",
                             email_subject,
                             email_body,
                             msg_request)
                # Write pending account to provision in REDIS
...
if __name__ == '__main__':
    ...
    try:
        ...
        logging.error("Unable to connect to the Redis {REDIS_SERVER}: %s", err)
        sys.exit(-1)
    ...
    try:
        logging.info("Connection and subscription to redis successful.")

        # Starting Telegram bot to check for new messages
        telegram_bot = threading.Thread(
                target=get_telegram_requests,
