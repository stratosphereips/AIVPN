...
def send_empty_capture_msg_via_email(msg_account_name,msg_address,SMTP_HOST,SMTP_USER,SMTP_PASSWORD):
    """ Function to send the message that profile expired to the user via email. """
    try:
        # Craft the email by hand
        body = "Thank you for using our Emergency VPN service. Unfortunately, there was no traffic captured in this Emergency VPN session. Please try again."
        headers = f"From: {SMTP_USER}\r\n"
        headers += f"To: {msg_address}\r\n"
        headers += f"Subject: [CivilSphere Emergency VPN] Emergency VPN Report: {msg_account_name}\r\n"
        email_message = headers + "\r\n" + body  # Blank line needed between headers and body
...
def send_profile_report_via_email(msg_account_name,msg_address,SMTP_HOST,SMTP_USER,SMTP_PASSWORD,PATH):
    ...
    try:
        ...
        email_message.add_header('To', msg_address)
        email_message.add_header('From', SMTP_USER)
        email_message.add_header('Subject', f"[Civilsphere Emergency VPN] Emergency VPN Report: {msg_account_name}\r\n")
            ...
            if item['type'] == 'message':
                ...
                if item['data'] == 'report_status':
                    ...
                    if 'send_expire_profile_email' in item['data']:
                        ...
                        continue
                    if 'send_empty_capture_email' in item['data']:
                        logging.info(f'Sending report that capture was empty to profile {msg_account_name} ({msg_address})')
                        if send_empty_capture_msg_via_email(msg_account_name,msg_address,IMAP_SERVER,IMAP_USERNAME,IMAP_PASSWORD):
